...
namespace NGE.Network
{
	...
	public abstract class SocketConnection : /*UnManagedResource,*/ IConnection
	{
        ...
        public event ConnectionGetDataCallback ConnectionGetData;
        ...
        public void noteGetData(MemoryStream ms)
        {
            ConnectionGetData(ms);
...
//			IPAddress[] addresses = Dns.GetHostEntry(hostName).AddressList;
			...
			for (int i = 0; i < addresses.Length; i++)
			{
				try
				{  //----这里有问题,这个Connect是异步的。
					return Connect(new IPEndPoint(addresses[i], port), secure, state);
				}
				catch (Exception ex)
				{
		...
		public int Connect(IPEndPoint endPoint, bool secure, object state)
		{
			...
			IAsyncResult asyncSend = m_socket.BeginConnect(endPoint, m_Connected, m_socket);
			bool success = asyncSend.AsyncWaitHandle.WaitOne( 5000, true );
            if ( !success )
            {
				m_socket.Close();
                UnityEngine.Debug.Log("Failed to SendMessage server.");
            }
		...
		public abstract void SendPacket(byte[] packet, bool encrypt_if_need);
	}
}
